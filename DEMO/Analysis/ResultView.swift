...
struct ResultView: View {
    ...
    private var assignmentSection: some View {
        if let patient {
            assignedCard(patient)
        } else if let onAssign {
            unassignedCard(onAssign)
        }
    }

    private func assignedCard(_ patient: Patient) -> some View {
        HStack(spacing: 10) {
    ...
    private func unassignedCard(_ onAssign: @escaping () -> Void) -> some View {
        VStack(spacing: 12) {
            HStack(spacing: 10) {
            ...
            Button(action: onAssign) {
                Label("Przypisz do pacjenta", systemImage: "person.badge.plus")
            }
